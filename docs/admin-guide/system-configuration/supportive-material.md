...
The information displayed by default is: the `code` of the registered [languages](admin-guide/system-configuration/languages.md), the `type` of materials and timestamps for the `creation` and `updates` of the records. At the top right corner of the listing you can also select which columns to display.
...
Editing and removing existing supportive materials is also possible by clicking on the three dots available on the last column.
...
- **Type**: You can filter supportive materials by the type they are enforced upon. You can select one or more types.<br/>*By default, no metric is selected.*
...
- **Language**: You select the specific available language. These materials can have translations available for all the registered [languages](admin-guide/system-configuration/languages.md).
- **Payload**: Here all the content that you want to add.
