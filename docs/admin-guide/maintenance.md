...
In **OpenCDMP**, [Elastic](https://www.elastic.co/) is used for indexing and searching [`Plans`](user-guide/plans/index.md) and [`Descriptions`](user-guide/descriptions/index.md) data (*more details about system architecture can be found [here](getting-started/architecture.md)*). That's why there are the following tasks for managing elastic indexes:
...
### Main Events
<!-- accounting disabled, kpi disabled -->
<!-- All main events sends data to [notification](optional-services/notifications/index.md), [annotation](optional-services/notifications/index.md), [accounting](optional-services/accounting-services/accounting-preview.md) (If is enabled for this installation) and [KPI](optional-services/kpi.md) (If is enabled for this installation) services.  -->
All main events sends data to [notification](optional-services/notifications/index.md) and [annotation](optional-services/notifications/index.md) services. 
...
<!-- accounting disabled -->
<!-- ### Accounting events
If [Accounting](optional-services/accounting-services/accounting-preview.md) service is enabled, you can **reset** the values of OpenCDMP **metrics**, by running the following tasks:
...
- **Send prefilling source accounting entries events**
- **Send reference accounting entries events**
...
- **Send language accounting entries events**
...
<!-- kpi disabled
### Indicator events
If [KPI](optional-services/kpi.md) service is enabled, to communicate with this, you can run the following tasks:
1. **Send indicator create entry events**: creates an KPI indicator with fixed schema and metadata based on [statistics](admin-guide/statistics.md)
...
10. **Send indicator point tenant entry events**: the existing indicator feeds tenants with data, which are called indicator points

:::warning
- `Send indicator create entry events` task is required to runs at least one time to display data in [Statistics Page](admin-guide/statistics.md).
- Until `Send indicator create entry events` is executed, the rest of indicator tasks will not have results
