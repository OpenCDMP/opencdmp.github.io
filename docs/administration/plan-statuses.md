...
In this page, there is a listing where you can view details about all the available statuses for a [plan](/docs/application/plans/index.md).
...
:::note
Before a a plan status can be used, it must be defined in the [Plan Workflow](/docs/application/plans/plan-workflow.md)
...
Editing and removing existing plan statuses is also possible by clicking on the tree dots available on the last column.
...
- **Internal Status**: Υou can filter the plan statuses that have the selected interan status.<br/>*By default, this option is not set.*
...
- **Internal Status**: The selection of internal status is very **critical** to the behavior of the plan. There are two internal types:
    ...
    <br/>*This is optional and if we don't set internal status, the plan is editable.*
...
- **Available Actions**: The available actions for the plan that can be executed when it has this status. The following actions are:
    - **[Depoit](/docs/application/plans/deposit-a-plan.md)**
    - **[Export](/docs/application/plans/exports.md)**
    - **[Evaluate](/docs/application/plans/evaluators.md)**
    <br/>*This is optional.*
- **Action**: The display name that the status change action in this status will have.<br/>*This is optional. if we don't set it, the status name will be displayed by default.*
- **Icon Action**: The icon that will appear in the available status change action in this status. The icon can either be `mat icon` from [angular material](https://material.angular.io/components/icon/overview) or an `image`.<br/>*This is optional and if we don't set it, a default icon will appear.*
