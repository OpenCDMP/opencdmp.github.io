...
For more information on how to customize a tenant's theme, check out [**Theming**](/docs/getting-started/configuration/frontend/theming.md)
</details>

<details>
  <summary>Deposit Plugins</summary>

Here we specify the [deposit plugins](/docs/supplementary-services/deposit-services.md), which will be available for this tenant. We have the option to disable system plugins by selecting the `Disable System Sources`. Also we can add plugins by pressing the `Add Source` button. For every plugin, we have to specify the following.
...
Here we specify the [evaluator plugins](/docs/supplementary-services/evaluator-services.md), which will be available for this tenant. We have the option to disable system plugins by selecting the `Disable System Sources`. Also we can add plugins by pressing the `Add Source` button. For every plugin, we have to specify the following.
...
Here we specify the file [file transformers plugins](/docs/supplementary-services/file-transformers.md), which will be available for this tenant. We have the option to disable system plugins by selecting the `Disable System Sources`. Also we can add plugins by pressing the `Add Source` button. For every plugin, we have to specify the following.
