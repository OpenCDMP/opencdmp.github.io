...
In this page, there is a listing where you can view details about all the available statuses for a [description](/docs/application/descriptions/index.md).
...
:::note
Before a a description status can be used, it must be defined in the [Description Workflow](/docs/application/descriptions/description-workflow.md)
...
Editing and removing existing description statuses is also possible by clicking on the tree dots available on the last column.
...
- **Internal Status**: Υou can filter the description statuses that have the selected interan status.<br/>*By default, this option is not set.*
...
- **Internal Status**: The selection of internal status is very **critical** to the behavior of the description. There are three internal types:
    ...
    <br/>*This is optional and if we don't set internal status, the description is editable.*
...
- **Available Actions**: The available actions for the description that can be executed when it has this status. The following actions are:
    - **[Export](/docs/application/descriptions/exports.md)**
    <br/>*This is optional.*
